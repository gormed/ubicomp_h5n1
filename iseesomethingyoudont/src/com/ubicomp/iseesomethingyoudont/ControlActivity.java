...
public class ControlActivity extends Activity implements OnTouchListener,
	...
	private static final boolean TOGGLE_ON_CLICK = false; // set=will toggle
															// systemUI
															// visibility upon
															// interaction.
															// Otherwise will
															...
															// visibility upon
															// interaction.
	private static final int HIDER_FLAGS = SystemUiHider.FLAG_HIDE_NAVIGATION; // The
																				// flags
																				// to
																				// pass
																				// to
																				// {@link
																				// SystemUiHider#getInstance}.
	private SystemUiHider mSystemUiHider; // The instance of the {@link
											// SystemUiHider} for this activity.
	...
	// RFID Emulation
	private static String[] handicap = { "Buggy", "Baum", "Telefonzelle",
			"Blumen", "Laterne", "Wasserspender", "M�lltonne", "Bank",
			"Schwarzes Loch", "Bierzelt" };
	private static float[] radius = { 0.5f, 1.0f, 0.75f, 2.0f, 3.6f, 5.7f,
			0.01f, 1.11f, 1.53f, 2.22f };
	private static float[] masse = { 10f, 20f, 30f, 40f, 50f, 60f, 70f, 80f,
			90f, 100f };

	// (Elastischer Eindringmodul � EIT),
	// Kompressionsmodul K - er beschreibt, welche allseitige Druck�nderung
	// n�tig ist,
	// um eine bestimmte Volumen�nderung hervorzurufen

	// Called when app is created (not when displayed)
	@Override
	// Main method of the android application
	...
	public void onPause() {
		super.onPause();//
		// Wenn man den kill au�erhalb des if macht, wird die app sofort bei
		// start gekillt
		if (eventToSpeechSynthesis != null) {
	...
	public void enableHardwareServices() {
		...
		// Creates the recognition of gestures -- VERURSACHT NOCH FEHLER WEGEN
		// DER SPEECH SYNTHESIS
		gestures = new GestureServices(gestureText, vibrator,
				eventToSpeechSynthesis, display, this);
		...
		locationServices = new LocationServices(vibrator,
				eventToSpeechSynthesis, this);
	...
	public void createTestRFID() {
		...
		// breite, hoehe, tiefe
		double[] size = { radius[rnd.nextInt(9)], radius[rnd.nextInt(9)],
				radius[rnd.nextInt(9)] };
		double mass = masse[rnd.nextInt(9)];
		String name = handicap[rnd.nextInt(9)];
		double longitude = locationServices.getUpdateLocation().getLo()
				+ rnd.nextFloat();
		double latitude = locationServices.getUpdateLocation().getLa()
				+ rnd.nextFloat();

		RFIDEvent event = new RFIDEvent(JsonRequester.getDeviceID(),
				RFIDEventType.NEW_TAG, name, size, mass, longitude, latitude);
	...
	private void checkRequired() {
		if (locationServices.checkGPSOn()
				&& networkInfo.isConnectedOrConnecting()) {
			if (audioManager.isWiredHeadsetOn()) {
				eventToSpeechSynthesis
				.speakTest("Willkommen!");
			} else {
				eventToSpeechSynthesis
				.speakTest("Willkommen! Zur optimalen Nutzung bitte Kopfhörer einstecken!");
			}
		} else {

			eventToSpeechSynthesis
					.speakTest("Die notwendigen Funktionen sind deaktiviert, bitte wenden sie sich an den Betreuer!");
